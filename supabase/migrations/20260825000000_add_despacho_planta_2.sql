...
-- 2. Añadir columnas a la tabla yard_operations en el esquema 'public' (por compatibilidad)
...
-- Eliminar restricciones CHECK sobre status en esquema 'public'
DO $$
DECLARE r record;
BEGIN
    FOR r IN
        ...
        FROM information_schema.table_constraints tc
        JOIN information_schema.constraint_column_usage ccu ON tc.constraint_name = ccu.constraint_name
        WHERE tc.table_name = 'yard_operations' AND tc.table_schema = 'public' AND tc.constraint_type = 'CHECK' AND ccu.column_name = 'status'
    LOOP
        EXECUTE 'ALTER TABLE public.yard_operations DROP CONSTRAINT IF EXISTS ' || quote_ident(r.constraint_name);
    END LOOP;
END $$;

-- Aplicar nueva restricción CHECK de status en esquema 'public'
ALTER TABLE public.yard_operations 
  ADD CONSTRAINT yard_operations_status_check 
  CHECK (status IN ('cita', 'planta_carga', 'en_ruta', 'espera', 'anden', 'completado'));

-- 3. Índices de rendimiento
CREATE INDEX IF NOT EXISTS idx_yard_operations_origin ON dock.yard_operations(origin);
CREATE INDEX IF NOT EXISTS idx_yard_operations_dispatch_time ON dock.yard_operations(dispatch_time);
